...
namespace Big::UserInterface
{
	...
	char* current_header;
	constexpr auto int_ban_delay = 40;
	int header_image = 1;
	int int_current_frame{};
	static auto int_banner_ticker = GetTickCount64();
	...
	void UIManager::DrawHeader()
	{
		...
		case HeaderType::Gradient:
			...
			break;
		case HeaderType::YTD:
			if (GetTickCount64() - int_banner_ticker >= int_ban_delay) {
				if (int_current_frame >= 149) {
					int_current_frame = 0;
				}
				else {
					int_current_frame++;
				}
				current_header = _strdup(("210a4957-6efc-44ca-89a0-9b91b3a00c3f-" + std::to_string(int_current_frame)).c_str());
				int_banner_ticker = GetTickCount64();
			}
			DrawSprite(
				"Axelrod",
				current_header,
	...
	{
		auto should_text = "";
		int should_draw_tick = 0;

		if (!std::strcmp(opt->GetRightText(), "~r~OFF") || !std::strcmp(opt->GetRightText(), "~g~ON") || (!std::strcmp(opt->GetRightText(), "~r~NO") || !std::strcmp(opt->GetRightText(), "~g~YES")))
		{
			if (!std::strcmp(opt->GetRightText(), "~r~OFF") || !std::strcmp(opt->GetRightText(), "~r~NO")) { should_text = ""; should_draw_tick = 1; }
			else { should_text = ""; should_draw_tick = 2; }

		}
		else
		{
			should_text = opt->GetRightText();
		}
		...
		if (should_draw_tick == 1)
		{
			DrawSprite(
				"commonmenu",
				selected ? "shop_box_blankb" : "shop_box_blank",
				m_PosX + m_Width / m_OptionPadding - 0.005f,
				m_DrawBaseY + m_OptionHeight / 2.f - GetTextHeight(m_OptionFont, m_OptionTextSize) / 1.5f + 0.013f,
				m_OptionTextSize * 0.085f,
				m_OptionTextSize * 0.15f,
				{255, 255, 255, 255},
				0);
		}
		else if (should_draw_tick == 2)
		{
			DrawSprite(
				"commonmenu",
				selected ? "shop_box_tickb" : "shop_box_tick",
				m_PosX + m_Width / m_OptionPadding - 0.005f,
				m_DrawBaseY + m_OptionHeight / 2.f - GetTextHeight(m_OptionFont, m_OptionTextSize) / 1.5f + 0.013f,
				m_OptionTextSize * 0.085f,
				m_OptionTextSize * 0.15f,
	...
	void UIManager::DrawFooter()
	{
		...
		float rotation = 0.f;
		auto texture = "shop_arrows_upanddown";
